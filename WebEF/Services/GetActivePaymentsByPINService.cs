...
{
    public class GetActivePaymentsByPINService : IGetActivePaymentsByPIN
    {
        private readonly IOptions<AppSettings> _appSettings;
        private ServiceManagerDbContext _db;
        public ISoapHeader XRoadHeader { get; set; }
        public GetActivePaymentsByPINService(IOptions<AppSettings> appSettings, ServiceManagerDbContext db)
        {
            _appSettings = appSettings;
            _db = db;
        }

        public GetActivePaymentsByPINResponse GetActivePaymentsByPIN(PINRequest request)
        {
            ...
            try
            {
                ...
                var apiMethod = "api/CISSA/GetActivePaymentsByPIN";
                var response = RequestHttp<GetActivePaymentsByPINResponse>(apiMethod, request);
                log.IsSuccess = true;
