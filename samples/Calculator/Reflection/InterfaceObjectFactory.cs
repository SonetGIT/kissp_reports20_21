...
namespace Calculator.Reflection
{
    ...
    public static class InterfaceImplementFactory
    {
        ...
        static InterfaceImplementFactory()
        {
            // Initialize an assembly and module builder for use for all generated classes
            AssemblyBuilder assemblyBuilder = AssemblyBuilder.DefineDynamicAssembly(Assembly.GetExecutingAssembly().GetName(), AssemblyBuilderAccess.Run);//appDomain.DefineDynamicAssembly(assemblyName, AssemblyBuilderAccess.RunAndSave);
            ...
            moduleBuilder = assemblyBuilder.DefineDynamicModule("Reflection");
        }
        /// <summary>
        ...
        public static void CreateTypeFor(Type type)
        {
            // Error checking...
        ...
        public static Type CreateInterface(string typeName)
        {
            var iBuilder = GetInterfaceBuilder(typeName);
            CreateInterfaceMethod(iBuilder, "p1", typeof(string));
            CreateInterfaceMethod(iBuilder, "p2", typeof(int));

            Type i = iBuilder.CreateType();
            return i;
        }
        private static TypeBuilder GetInterfaceBuilder(string typeName)
        {
        ...
        private static void CreateInterfaceMethod(TypeBuilder tb, string propertyName, Type propertyType)
        {
            var ctorParams = new Type[] { typeof(string) };
            var classCtorInfo = typeof(XRoadServiceAttribute).GetConstructor(ctorParams);

            CustomAttributeBuilder myCABuilder2 = new CustomAttributeBuilder(classCtorInfo, new object[] { "MyService" });

            MethodBuilder methodBuilder = tb.DefineMethod(/*"get_" + */propertyName, MethodAttributes.Virtual | MethodAttributes.Abstract | MethodAttributes.Public, propertyType, new Type[] { typeof(string) });
            methodBuilder.SetCustomAttribute(myCABuilder2);
